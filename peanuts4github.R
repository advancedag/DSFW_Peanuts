...
api.key <- 'XXXXXXXXXXXXXXX' #insert your API hey here
api.url4acres <- paste(
...
dat<-subset(peanutHarvest, year==2017) 
...
for(i in 1:nlevels(factor(peanutPlantProg$state_name))){
  ...
  last5yearsp<-subset(plantdat, year>=2015)
  ...
  numRows<-as.numeric(max(levels(factor(DSFWdat$begin_code))))
  ...
  numWeeks<-end85p-begin15p+1
    ...
    hlast5years<-subset(hdat, year>=2015) 
